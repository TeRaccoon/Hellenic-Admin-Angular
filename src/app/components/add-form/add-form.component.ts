...
import { keyedData, settings, data, keyedAddress, formState } from '../../common/types/forms/types';
...
export class AddFormComponent {
  ...
  async isAutoGenerated() {
    switch (this.tableName) {
      ...
      case 'stocked_items':
        await this.replaceAutoGeneratedFields('barcode');
        break;
    }
    return false;
  }
  ...
  async replaceAutoGeneratedFields(field: string) {
    switch (field) {
      case 'title':
        let nextInvoiceId = await this.dataService.processGet('next-id', { filter: this.tableName });
      ...
      case 'account_code':
        let nextSupplierAccountCode = await this.dataService.processGet('next-supplier-account-code', { filter: this.tableName });
      ...
      case 'reference':
        let nextId = await this.dataService.processGet('next-id', { filter: this.tableName });
        if (nextId != null) {
          this.autoGeneratedReplacement[field] = { value: 'SPL' + String(nextId).padStart(6, '0') };
        }
        break;
  ...
  async buildForm() {
    if (this.shouldDisplayItemWidget()) {
      ...
      if (this.tableName == 'supplier_invoices') {
        ...
        this.invoiceId = await this.dataService.processGet('next-id', { filter: 'supplier_invoices' });
      } else {
        this.addItemForm = this.fb.group({
          item_id: ['', [Validators.required]],
          quantity: ['', [Validators.required]],
  ...
  async updateSelectedReplacementDataFromKey(dataId: number, dataValue: string, key: string, field: string, alt: boolean) {
    ...
    } else if (this.tableName == 'supplier_invoices' && field == 'item_id') {
      let lastPurchasePrice = await this.dataService.processGet('last-purchase-price', { filter: dataId });
      if (lastPurchasePrice.length > 0 && lastPurchasePrice[0] != null) {
        this.addItemForm.get('purchase_price')?.setValue(lastPurchasePrice[0]);
      }
    }
    if ((this.tableName == 'stocked_items' || this.tableName == 'supplier_invoices') && (field == 'item_id' || field == 'expiry_date')) {
      if (this.addForm.get('item_id')?.value != null && this.addForm.get('expiry_date')?.value != null) {
        let item = await this.dataService.processGet('items', { filter: this.addForm.get('item_id')?.value });
        let stockCode = item['stock_code'];
        this.addForm.get('barcode')?.setValue(stockCode + this.addForm.get('expiry_date')?.value.toString());
      }
    }
  ...
  async addItem(event: Event) {
    ...
    let submissionResponse = await this.dataService.submitFormData(this.addItemForm.value);
  ...
  inputHasError(field: string) {
    return this.addForm.get(field)?.invalid && this.formState.submissionAttempted;
  }
