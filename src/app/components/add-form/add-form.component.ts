...
export class AddFormComponent {
  ...
  async replaceAutoGeneratedFields(field: string) {
    switch (field) {
      case 'title':
        let nextInvoiceId = await lastValueFrom(this.dataService.processData('next-id', this.tableName));
      ...
      case 'reference':
        let nextId = await lastValueFrom(this.dataService.processData('next-id', this.tableName));
        if (nextId != null) {
  ...
  async buildForm() {
    if (this.shouldDisplayItemWidget()) {
      ...
      if (this.tableName == 'supplier_invoices') {
        ...
        this.invoiceId = await lastValueFrom(this.dataService.processData('next-id', 'supplier_invoices'));
      } else {
        this.addItemForm = this.fb.group({
          item_id: ['', [Validators.required]],
          quantity: ['', [Validators.required]],
  ...
  async updateSelectedReplacementDataFromKey(dataId: number, dataValue: string, key: string, field: string, alt: boolean) {
    ...
    if (this.tableName == "invoices" && field == "customer_id") {
      ...
      let lastPurchasePrice = await lastValueFrom(this.dataService.processData('last-purchase-price', String(dataId)));
    ...
    if ((this.tableName == 'stocked_items' || this.tableName == 'supplier_invoices') && (field == 'item_id' || field == 'expiry_date')) {
      if (this.addForm.get('item_id')?.value != null && this.addForm.get('expiry_date')?.value != null) {
        let item = await lastValueFrom(this.dataService.processData('items', this.addForm.get('item_id')?.value));
