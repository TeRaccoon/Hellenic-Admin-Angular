...
export class AddFormComponent {
  ...
  async buildForm() {
    ...
    formDataArray = [...formDataArray].sort(
      (
        a: [string, { inputType: string }],
        b: [string, { inputType: string }],
      ) => {
  ...
  private async handleCreditNotesCustomers(dataId: number) {
    ...
    this.updateReplacementDataForInvoices(
      this.invoiceDetails,
      'title',
      'Invoice',
    );
  ...
  async generateBarcode(alt: boolean = false) {
    ...
    if (
      ...
      const barcode = `${stockCode}-${form.get('expiry_date')
        ?.value}-${this.generateRandomString(7)}`;
  ...
  dropdownChanged(
    ...
    this.performSearch(key, filter, derivedField, false, false);
  }
  ...
  canDisplayInputField(key: string) {
    ...
    if (this.tableName === 'credit_notes_customers') {
      if (
        key === 'Restock' &&
        this.addForm.get('invoiced_item_id')?.value == ''
      ) {
        return false;
      }
      if (
        key === 'Invoiced Item ID' &&
        this.addForm.get('invoice_id')?.value == ''
      ) {
        return false;
      }
    }
