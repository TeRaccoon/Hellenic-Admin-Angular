...
import { DISPLAY_INPUT_FIELD_TABLE_MAP_EXCLUSIONS, DISPLAY_PRICE_WARNING_TABLES, FULLSCREEN_TABLES } from '../consts';
...
export class AddFormComponent implements OnInit, OnDestroy {
  ...
  debounceSearch: (key: string, filter: string, field: string | null, text: boolean | null, alt: boolean) => void =
    _.debounce(
      (key: string, filter: string, field: string | null, text = false, alt = false) =>
  ...
  selectOpen: Record<string, { opened: boolean | null }> = {};
  ...
  async replaceAmbiguousData() {
    ...
    Object.keys(this.replacementData).forEach((key) => {
      ...
      if (this.filteredReplacementData[key].data.length > 0 && this.filteredReplacementData[key].data[0]?.id != null) {
  ...
  async buildForm() {
    ...
    formDataArray = [...formDataArray].sort(
      (a: [string, { inputType: string }], b: [string, { inputType: string }]) => {
    ...
    if (this.addForm.valid) {
      if (this.tableName != 'categories') {
        ...
        if (validationResult !== false) {
          this.submissionWithImage(validationResult.id, validationResult.name);
        } else {
  ...
  private updateReplacementDataForInvoices(invoicesDetails: any[], key = 'title', replacementKey = 'Invoice ID') {
    this.filteredReplacementData[replacementKey].data = this.replacementData[replacementKey].data = invoicesDetails.map(
      (i: any) => ({
        id: i.id,
        replacement: i[key],
      })
  ...
  async generateBarcode(alt = false) {
    ...
    if (form.get('item_id')?.value != null && form.get('expiry_date')?.value != null) {
      ...
      const barcode = `${stockCode}-${form.get('expiry_date')?.value}-${this.generateRandomString(7)}`;
  ...
  filterDropSelect(key: string, event: any, includeField: boolean, field: string | null = null, alt = false) {
    ...
    if (this.replacementData[key]?.data.length > 0) {
      const derivedField = includeField ? this.mappedFormData.get(key)!.field : field;
  ...
  performSearch(key: string, filter: string, field: string | null, text: boolean | null = false, alt = false) {
  ...
  changeSaleType(event: Event) {
    ...
    if (this.saleType == SaleType.Cash) {
      ...
      const replacement = data.data.filter((data: any) => data.replacement == 'N/A: Counter Sale')[0];
  ...
  shouldDisplayItemWidget() {
    const includedTables = ['invoices', 'supplier_invoices'];
    return includedTables.includes(this.tableName);
  }
