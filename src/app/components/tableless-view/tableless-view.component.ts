    ...
    if (this.vatReturnForm.valid) {
      ...
      const data = {
        ...
        previousAdjustments: [
          ...
          0,
          0,
          this.vatReturnForm.get('vat-reclaimed-previous')?.value,
          0,
          this.vatReturnForm.get('total-sales-previous')?.value,
          this.vatReturnForm.get('total-purchases-previous')?.value,
          '',
          '',
        ],
        newAdjustments: [
          this.vatReturnForm.get('vat-due-new')?.value,
          0,
          0,
          this.vatReturnForm.get('vat-reclaimed-new')?.value,
          0,
          this.vatReturnForm.get('total-sales-new')?.value,
          this.vatReturnForm.get('vat-purchases-new')?.value,
          '',
          '',
        ],
        fuelScaleCharge: [
          this.vatReturnForm.get('vat-due-fuel')?.value,
          0,
          0,
          0,
          0,
          this.vatReturnForm.get('total-sales-fuel')?.value,
          0,
          0,
          0,
        ],
  ...
  async addReturnToDatabase(returnData: any, period: string, date: string, notes: string) {
    if ((await this.vatReturnExists(period)) && !this.submitAttempted) {
      ...
      if (await this.vatReturnExists(period)) {
        await this.dataService.processGet('delete-vat-returns-by-group-id', {
          filter: period,
        });
      }
