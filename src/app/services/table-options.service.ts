...
export class TableOptionsService {
    ...
    switch (tableName) {
      ...
      case 'invoices':
        if (currentRow['status'] == 'Complete') {
          this.formService.setMessageFormData({
            title: 'Error',
            message: 'You cannot delete this invoice because it has been marked completed!',
          });
          return false;
        }
        break;
  ...
  async calculateDistance(selectedRows: any[], row: any) {
    ...
    const coordinates = await this.dataService.processGet('calculate-distance', {
  ...
  addRow(editable: any, values: any, tableName: string) {
    ...
    this.formService.processAddFormData(addFormData, null, this.formService.constructFormSettings(tableName));
